...
AS
;WITH SHARETRACKERBUY(SHARES_ID, AMOUNT, SHARE_QTY)
AS
(
SELECT HDR.SHARES_TRACKER_HEADER_ID AS SHARES_ID, ISNULL(SUM((BUY.SHARES_PRICE_AMT * BUY.SHARES_QUANTITY)+ ISNULL(BUY.SHARES_CHARGES_AMT,0)),0) AS AMOUNT,
SUM(BUY.SHARES_QUANTITY) FROM SHARES_TRACKER_HEADER HDR 
...
SHARETRACKERSELL(SHARES_ID, AMOUNT, SHARE_QTY)
AS
(
SELECT HDR.SHARES_TRACKER_HEADER_ID AS SHARES_ID, ISNULL(SUM((SEL.SHARES_PRICE_AMT * SEL.SHARES_QUANTITY) - ISNULL(SEL.SHARES_CHARGES_AMT,0)),0) AS AMOUNT,
SUM(SEL.SHARES_QUANTITY) FROM SHARES_TRACKER_HEADER HDR 
...
SELECT HD.SHARES_TRACKER_HEADER_ID, HD.SHARES_NAME , 
CASE
WHEN A.SHARE_QTY != B.SHARE_QTY THEN NULL
 ELSE (B.AMOUNT - A.AMOUNT)
 END AS SHARE_AMT,
CASE
WHEN A.SHARE_QTY != B.SHARE_QTY THEN 'EQUAL'
